...
from rag_lib.config.defaults import language_model_defaults
from rag_lib.config.enums import AsyncType, AuthType, ModelType
...
from rag_lib.language_model.factory import ModelFactory
...
class LanguageModelConfig(BaseModel):
    ...
    def _validate_api_key(self) -> None:
        ...
        if (self.auth_type == AuthType.OnPromise) and (
            self.on_promise_model is not None and self.on_promise_model.strip() != ""
        ):
            logger.info(f"on_promise_model not found.")
